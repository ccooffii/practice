window.dom = {
    ...
    attr (node, attrName, attrValue) {
        if (arguments.length === 3) //eg dom.attr(div,'name','Jeremy') 
        node.setAttribute(attrName, attrValue);
        else if (arguments.length === 2) //eg dom.attr(div,'name')
    ...
    html (node, string) {
        if (arguments.length === 2) //eg: dom.text(div,'<span> Hello </span>')
        node.innerHTML = string;
        else if (arguments.length === 1) //eg: dom.text(div,'<span> Hello </span>')
    ...
    style (node, name, value) {
        ...
        else if (arguments.length === 2) //eg: dom.style(div,'color')
    ...
    next (node) {
        ...
        while(temp && temp.nodeType === 3)temp = temp.nextSibling;
        return temp;
    },
    //get previous sibling node
    previous (node) {
        let temp = node.previousSibling;
        while(temp && temp.nodeType === 3)temp = temp.previousSibling;
        return temp;
    },
    //travel nodes
    each (nodeList, fn) {
